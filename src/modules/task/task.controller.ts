...
import { validateTask, validateTaskUpdate, validateTaskPatch } from './task.validation.ts';
import { getAllActiveTasks, getTaskByIdWithStatus, getTasksByStatusCode,  isValidStatus, createNewTask, updateExistingTask, addTaskComment,  softDeleteTaskById } from './task.util.ts';
...
const createTask = async (req: AuthenticatedRequest, res: Response): Promise<Response |void> => {
  ...
  try {
    ...
    const task = await createNewTask(
      { name, description, status: taskStatus, dueDate: due_date },
      teamMembers,
      req.user.id
    );
...
const patchTask = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { id } = req.params;
    ...
    if (req.user.isAdmin) {
      ...
      if (status && !comment) {
        ResponseBuilder.error(
          res,
          400,
          messages.ERROR.COMMENT_REQUIRED_FOR_STATUS,
          'Comment is required when updating status for non-admin users'
        );
        return;
      }
      ...
      ResponseBuilder.success(
        ...
        updates.includes('status')
          ...
          : messages.SUCCESS.COMMENT_ADDED,
        { task }
      );
    }
  } catch (error: any) {
